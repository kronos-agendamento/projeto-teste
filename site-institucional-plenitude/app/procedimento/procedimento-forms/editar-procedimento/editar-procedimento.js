document.addEventListener("DOMContentLoaded", async function () {
  ...
  try {
    ...
    document.getElementById("duracao-colocacao-minutos").value = colocacao.minutos;

    const manutencao = parseTempoMinutos(procedimento.tempoManutencao);
    document.getElementById("duracao-manutencao-horas").value = manutencao.horas;
    document.getElementById("duracao-manutencao-minutos").value = manutencao.minutos;
    ...
    document.getElementById("duracao-retirada-minutos").value = retirada.minutos;
  } catch (error) {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelector(".save-button").addEventListener("click", async function (event) {
      ...
      const especificacao = {
        ...
        precoColocacao: parseValue(document.getElementById("valor-colocacao").value) || 0,
        precoManutencao: parseValue(document.getElementById("valor-manutencao").value) || 0,
        precoRetirada: parseValue(document.getElementById("valor-retirada").value) || 0,
      ...
      try {
        ...
        if (response.ok) {
          ...
          showNotification("Erro ao atualizar especificação! Verifique os campos.", true);
        }
      } catch (error) {
  ...
  document.querySelector(".save-button").addEventListener("click", async function (event) {
    ...
    const updateEspecificacao = fetch(`${baseUrl}/especificacoes/${idEspecificacao}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(especificacao),
    });

    const updateProcedimento = fetch(`${baseUrl}/procedimentos/${idProcedimento}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
