document.addEventListener('DOMContentLoaded', function () {
    ...
    const telefoneEmergencial = document.getElementById('emergencia').value.replace(/\D/g, ''); // Remove tudo que não for número
        ...
        try {
            let enderecoId;
            const verificarEnderecoResponse = await fetch(`http://localhost:8080/api/enderecos/buscar-por-tudo/${logradouro}/${numero}/${cep}`);
            ...
            if (verificarEnderecoResponse.ok) {
                ...
                const enderecoResponse = await fetch('http://localhost:8080/api/enderecos/cadastrar', {
                    method: 'POST',
                    headers: {
                ...
                const enderecoData = await fetch(`http://localhost:8080/api/enderecos/buscar-por-tudo/${logradouro}/${numero}/${cep}`);
                if (!enderecoData.ok) {
            ...
            try {
                const verificarComplementoResponse = await fetch(`http://localhost:8080/api/complementos/buscar-por-endereco-complemento/${enderecoId}/${complemento}`);
                if (!verificarComplementoResponse.ok) {
                    if (verificarComplementoResponse.status === 404) {
                        // Se o complemento não existe, cria um novo
                        const complementoResponse = await fetch('http://localhost:8080/api/complementos/cadastrar', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json'
                            },
                ...
                if (complementoError.message.includes('500')) {
                    ...
                    console.log("Complemento cadastrado com sucesso após falha na verificação.");
                } else {
            ...
            const usuarioResponse = await fetch('http://localhost:8080/usuarios/cadastro-usuario', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
